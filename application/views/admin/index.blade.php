...
<div class="well admin">
	...
	<div class="admin-content">
		...
		<input type="checkbox" name="left_panel_show" @if($settings['left_panel_show'] == 1) checked @endif >
		<br>
		<label for="left_panel_text">{{ Lang::line('locale.admin_left_advertisement_code')->get($language) }}</label>
		<textarea class="code" rows="" cols="" name="left_panel_text">{{$settings['left_panel_text']}}</textarea>
...
<div class="well admin">
	...
	<div class="admin-content">
		...
		<input type="checkbox" name="right_panel_show" @if($settings['right_panel_show'] == 1) checked @endif >
		<br>
		<label for="right_panel_text">{{ Lang::line('locale.admin_right_advertisement_code')->get($language) }}</label>
		<textarea class="code" rows="" cols="" name="right_panel_text">{{$settings['right_panel_text']}}</textarea>
		<input type="submit" class="blue-button" value="{{ Lang::line('locale.save')->get($language) }}">
		{{ Form::close() }}
	</div>
</div>
